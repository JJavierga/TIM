...
from src.timV5 import TIM, TIM_ADM, TIM_GD
...
class Evaluator:
    ...
    def run_task_inductive(self, task_dic, model, callback, n_ways):
        ...
        y_s = y_s.long().squeeze(2).cpu()
        y_q = y_q.long().squeeze(2).cpu()

        # Perform normalizations required
        support = F.normalize(support, dim=2)
        query = F.normalize(query, dim=2)

        support_feat_cpu = support.detach().cpu().numpy()
        query_feat_cpu = query.detach().cpu().numpy()
        support_lab_cpu = y_s.cpu().numpy()
        query_lab_cpu = y_q.cpu().numpy()

        # Set classifier
        self.Classifier()
        self.clf.fit(support_feat_cpu[0], support_lab_cpu[0])
        n_tasks, n_images, n_feats = query_feat_cpu.shape
        ...
        for task in range(n_tasks):
            query_ys_pred = self.clf.predict(query_feat_cpu[task])

            if(n_ways==5):
                table = confusion_matrix(query_lab_cpu[task], query_ys_pred, labels=[0,1,2,3,4])

            elif(n_ways==2):
                table = confusion_matrix(query_lab_cpu[task], query_ys_pred, labels=[0,1])

            elif(n_ways==7):
                table = confusion_matrix(query_lab_cpu[task], query_ys_pred, labels=[0,1,2,3,4,5,6])
                #labels=["cracks","efflorescence","moss","no defect","painting","scaling","spalling"])

            #acc += (query_ys_pred == query_lab_cpu[task]).sum()
            #assigned.append(query_ys_pred)
            if task==0:
                global_table = table
            else:
                global_table += table
    ...
    def run_task_transductive(self, task_dic, model, callback, n_ways):
        ...
        # Run adaptation
        prob, assigned, acc = tim_builder.run_adaptation(support=support, query=query, y_s=y_s, y_q=y_q, callback=callback, n_ways=n_ways)

        n_tasks, n_images, n_feats = query.shape
        for task in range(n_tasks):
            y_q_cpu = y_q.cpu().numpy()
            if(n_ways==5):
                table = confusion_matrix(y_q_cpu[task], assigned[task], labels=[0,1,2,3,4])

            elif(n_ways==2):
                table = confusion_matrix(y_q_cpu[task], assigned[task], labels=[0,1])

            elif(n_ways==7):
                table = confusion_matrix(y_q_cpu[task], assigned[task], labels=[0,1,2,3,4,5,6])
        ...
        for i in range(n_ways):
            for j in range(n_ways):
                global_table[i,j]=global_table[i,j]/n_tasks*100
