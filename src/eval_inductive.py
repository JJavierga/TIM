...
class Evaluator:
    ...
    def run_full_evaluation(self, model, model_path, model_tag, shots, method, callback, target_split_dir, checking, query_shots, number_tasks, n_ways):
        ...
        for shot in shots:
            ...
            assigned, acc, tp, fp, tn, fn = self.run_task(task_dic=tasks,
                                model=model,
                                callback=callback,
                                n_ways=n_ways)
            n_img = 0
    ...
    def run_task(self, task_dic, model, callback, n_ways):
        ...
        for task in range(n_tasks):
            ...
            if(n_ways==5):
                true_positives += ((query_ys_pred != 2) * (query_lab_cpu[task] != 2)).sum()
                false_positives +=  ((query_ys_pred != 2) * (query_lab_cpu[task] == 2)).sum()
                true_negatives += ((query_ys_pred == 2) * (query_lab_cpu[task] == 2)).sum()
                false_negatives +=  ((query_ys_pred == 2) * (query_lab_cpu[task] != 2)).sum()

            elif(n_ways==2):
                true_positives += ((query_ys_pred != 1) * (query_lab_cpu[task] != 1)).sum()
                false_positives +=  ((query_ys_pred != 1) * (query_lab_cpu[task] == 1)).sum()
                true_negatives += ((query_ys_pred == 1) * (query_lab_cpu[task] == 1)).sum()
                false_negatives +=  ((query_ys_pred == 1) * (query_lab_cpu[task] != 1)).sum()
        ...
        acc = acc / (n_images*n_tasks)

        return  assigned, acc, true_positives, false_positives, true_negatives, false_negatives
